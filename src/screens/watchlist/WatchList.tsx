...
export const WatchList: React.FC = () => {
  const [invoke, setinvokes] = useState(0);
  const navigate = useNavigate();
  const { watchList, removeFromWatchList } = useTgStorage();
  ...
  useEffect(() => {
    setEmpty(watchList.filter((x: any) => !!x).length === 0);
  }, [watchList]);

  return (
    <>
      <div className={styles.panel}>
        <h2 className={styles.mainTitle}>WatchList</h2>
        ...
        {isEmpty && (
          <>
            ...
            >
              Go find movie to watch
            </button>
          </>
        )}

        {movies
          .filter((item: any) => watchList.includes(item.id))
            ...
            <div key={i} className={styles['panel-item']}>
              ...
              <div className={styles.annotation}>
                <div className={styles.description}>
                  ...
                  >
                    <span>Delete from watch list</span>
                    <svg
